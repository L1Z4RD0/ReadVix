import { Yn as shallowRef } from "./vue.runtime.esm-bundler-CJdF6Oq_.js";
import { i as createRouter$1, o as createWebHashHistory$1, r as createMemoryHistory$1, s as createWebHistory$1, v as parseQuery } from "./vue-router-7z9iCsBP.js";
//#region node_modules/@ionic/vue-router/dist/index.js
var createLocationHistory = () => {
	...
	const update = (routeInfo) => {
		...
		if (locationIndex > -1) locationHistory.splice(locationIndex, 1, routeInfo);
		const tabArray = tabsHistory[routeInfo.tab || ""];
		if (tabArray) {
			const tabIndex = tabArray.findIndex((x) => x.id === routeInfo.id);
			if (tabIndex > -1) tabArray.splice(tabIndex, 1, routeInfo);
			else tabArray.push(routeInfo);
		} else if (routeInfo.tab) tabsHistory[routeInfo.tab] = [routeInfo];
	};
	const pop = (routeInfo) => {
	...
	const addRoute = (routeInfo) => {
		const tabHistory = getTabsHistory(routeInfo.tab);
		if (tabHistory) {
			if (tabHistory[tabHistory.length - 1] && tabHistory[tabHistory.length - 1].id === routeInfo.id) tabHistory.pop();
			tabHistory.push(routeInfo);
		}
		locationHistory.push(routeInfo);
	};
	...
	*/
	const clearHistory = (routeInfo) => {
		if (routeInfo) {
			...
			*/
			...
			if (tab && tabHistory) clearTabHistory(tab);
			else for (const tab in tabsHistory) clearTabHistory(tab);
		} else {
			for (const tab in tabsHistory) tabsHistory[tab] = [];
	...
	const getTabsHistory = (tab) => {
		...
		if (tab) {
			...
			if (!history) history = tabsHistory[tab] = [];
		}
		return history;
	};
	...
	*/
	const current = (initialHistory, currentHistory) => {
		return locationHistory[currentHistory - initialHistory] || last();
	};
	...
	*/
	...
	const getFirstRouteInfoForTab = (tab) => {
		...
		if (tabHistory) return tabHistory[0];
	};
	const getCurrentRouteInfoForTab = (tab) => {
		const tabHistory = getTabsHistory(tab);
		if (tabHistory) return tabHistory[tabHistory.length - 1];
	};
	/**
	...
	*/
	const findLastLocation = (routeInfo, delta = -1) => {
		...
		if (routeInfos) if (delta < -1) return routeInfos[routeInfos.length - 1 + delta];
		else for (let i = routeInfos.length - 2; i >= 0; i--) {
			const ri = routeInfos[i];
			if (ri) {
				if (ri.pathname === routeInfo.pushedByRoute) return ri;
			}
		}
		for (let i = locationHistory.length - 2; i >= 0; i--) {
			const ri = locationHistory[i];
			if (ri) {
				if (ri.pathname === routeInfo.pushedByRoute) return locationHistory[i + 1 + delta];
			}
		}
	};
	return {
...
var createIonRouter = (opts, router) => {
	...
	*/
	...
	if (typeof document !== "undefined") document.addEventListener("ionBackButton", (ev) => {
	...
	const handleNavigateBack = (defaultHref, routerAnimation) => {
		...
		if (routeInfo && routeInfo.pushedByRoute) {
			...
			if (prevInfo) {
				...
				if (routeInfo.lastPathname === routeInfo.pushedByRoute || prevInfo.pathname === routeInfo.pushedByRoute && !routeInfo.tab && !prevInfo.tab) router.back();
				else
 /**
	...
	const handleNavigate = (path, routerAction, routerDirection, routerAnimation, tab) => {
		...
		if (routerAction === "push") router.push(path);
 ...
 /**
		...
		*/
		if (incomingRouteParams.routerAction === "replace") leavingLocationInfo = locationHistory.current(initialHistoryPosition, currentHistoryPosition);
			...
			*/
			if (action === "replace") locationHistory.clearHistory();
		} else {
			/**
		...
		else leavingLocationInfo = currentRouteInfo;
		if (!leavingLocationInfo) leavingLocationInfo = {
			pathname: "",
			search: ""
		};
		if (leavingLocationInfo.pathname + leavingLocationInfo.search !== location.fullPath) {
			if (!incomingRouteParams) {
				if (action === "replace") incomingRouteParams = {
					...
					} else incomingRouteParams = {
						routerAction: "pop",
						routerDirection: "none"
					};
				}
				if (!incomingRouteParams) incomingRouteParams = {
					routerAction: "push",
					routerDirection: direction || "forward"
				};
			}
			let routeInfo;
			if (incomingRouteParams === null || incomingRouteParams === void 0 ? void 0 : incomingRouteParams.id) routeInfo = Object.assign(Object.assign({}, incomingRouteParams), { lastPathname: leavingLocationInfo.pathname });
			else {
				const isPushed = incomingRouteParams.routerAction === "push" && incomingRouteParams.routerDirection === "forward";
				routeInfo = Object.assign(Object.assign({ id: generateId("routeInfo") }, incomingRouteParams), {
				...
				if (isPushed) routeInfo.pushedByRoute = leavingLocationInfo.pathname !== "" ? leavingLocationInfo.pathname : void 0;
			...
			if (historySize > historyDiff || historySize === historyDiff && historySize > 0 && action === "replace") {
				/**
			...
			} else locationHistory.add(routeInfo);
			/**
			* If we recently reset the location history
			* then we also need to update the initial
			* history position.
			*/
			if (locationHistory.size() === 1) initialHistoryPosition = routeInfo.position;
	...
	const resetTab = (tab) => {
		...
		*/
		...
		if (routeInfo) router.go(routeInfo.position - currentHistoryPosition);
	};
	const changeTab = (tab, path) => {
		...
		if (routeInfo) {
			...
			* tab you are on.
			*/
			if (routeInfo.pathname === pathname) router.push({
		...
		} else handleNavigate(pathname, "push", "none", void 0, tab);
	};
	/**
...
var createViewStacks = (router) => {
	...
	*/
	...
	const findViewItemInStack = (path, stack) => {
		return stack.find((viewItem) => {
			if (viewItem.pathname === path) return viewItem;
		});
	};
	const findViewItemByPath = (path, outletId, mustBeIonRoute = false) => {
		const matchView = (viewItem) => {
			if (mustBeIonRoute && !viewItem.ionRoute || path === "") return false;
			const findMatchedRoute = router.resolve(path).matched.find((matchedRoute) => matchedRoute === viewItem.matchedRoute);
			if (findMatchedRoute) {
				if (findMatchedRoute.path.includes(":") && path !== viewItem.pathname) return false;
				return viewItem;
			}
		};
		if (outletId) {
		...
		} else for (const outletId in viewStacks) {
			const stack = viewStacks[outletId];
	...
	const add = (viewItem) => {
		...
		if (!viewStacks[outletId]) viewStacks[outletId] = [viewItem];
		else viewStacks[outletId].push(viewItem);
	};
	const remove = (viewItem, outletId) => {
		if (!outletId) throw Error("outletId required");
		const viewStack = viewStacks[outletId];
		if (viewStack) viewStacks[outletId] = viewStack.filter((item) => item.id !== viewItem.id);
	};
	const getChildrenToRender = (outletId) => {
		if (viewStacks[outletId]) return viewStacks[outletId].filter((v) => v.mount);
		return [];
	};
	/**
	...
	*/
	const unmountLeavingViews = (outletId, viewItem, delta = 1) => {
		...
		for (let i = startIndex + 1; i < startIndex - delta; i++) {
			...
			viewItem.ionPageElement = void 0;
	...
	*/
	const mountIntermediaryViews = (outletId, viewItem, delta = 1) => {
		...
		if (!viewStack) return;
		const startIndex = viewStack.findIndex((v) => v === viewItem);
		for (let i = startIndex + 1; i < startIndex + delta; i++) viewStack[i].mount = true;
	};
	return {
		unmountLeavingViews,
...
var createWebHistory = (base) => createWebHistory$1(base);
var createWebHashHistory = (base) => createWebHashHistory$1(base);
var createMemoryHistory = (base) => createMemoryHistory$1(base);
//#endregion
